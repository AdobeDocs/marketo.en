...
1. Search for the trigger and then drag and drop it onto the canvas.
   ...
   >[!NOTE]
   >
   >A smart campaign with triggers runs in Trigger mode. It runs
   >on one person at a time based on events triggered and any
   >additional filters.
...
1. Click the drop-down and choose an operator (e.g. **is**, **is not**, etc.) for the trigger you chose.
   ...
   >triggers go at the top and when triggered, only people who
   >satisfy the filter criteria go through the flow.
...
>[!NOTE]
>
>With multiple triggers, a person goes through the flow if ANY
>of the triggers are activated.
>
>To run the campaign on a set of people all at the same time,
>learn how to [Define Smart List for Smart Campaign | Batch](https://docs.marketo.com/display/DOCS/Define+Smart+List+for+Smart+Campaign+%7C+Batch).
