---
unique-page-id: 2360317
description: Assign Person Partitions to Workspaces - Marketo Docs - Product Documentation
title: Assign Person Partitions to Workspaces
---

# Assign Person Partitions to Workspaces {#assign-person-partitions-to-workspaces}

Assign Person Partitions to Workspaces - Marketo Docs - Product Documentation

Here's how to edit the person partition / workspace assignments:

>[!NOTE]
>
>**FYI**
>
>Marketo is now standardizing language across all subscriptions, so you may see lead/leads in your subscription and person/people in docs.marketo.com. These terms mean the same thing; it does not affect article instructions. There are some other changes, too. [Learn more](http://docs.marketo.com/display/DOCS/Updates+to+Marketo+Terminology).

>[!NOTE]
>
>**Admin Permissions Required**

>[!NOTE]
>
>**Prerequisites**
>
>[Create a New Workspace](create-a-new-workspace.md)

>[!CAUTION]
>
>Workspaces and Person Partitions can be complex. Please contact [Marketo support](http://support.marketo.com/) for help in setting them up.

##### 1. Under Admin, click on Workspaces & Partitions.  {#assignpersonpartitionstoworkspaces-underadmin-clickonworkspaces&partitions}

![](assets/image2014-9-17-11-3a13-3a24.png)

1. Select your workspace and click **Edit Workspace**.

   ![](assets/two-3.png)

1. Edit the person partition information you want to change.

   >[!NOTE]
   >
   >The All Person Partitions checkbox indicates that this workspace has access to all person partitions in the system.

   ![](assets/three-3.png)

   Primary person partitions are the default where all peoples will be entered. Use [flow steps](../../../../welcome-to-marketo-docs/product-docs/core-marketo-concepts/smart-campaigns/flow-actions/use-add-choice-in-a-flow-step.md) or [assignment rules](assigning-person-partitions-with-assignment-rules.md) to move people around. 

1. Click **Save**.

   ![](assets/four-3.png)

   After saving, you should see the changes!

   ![](assets/image2014-9-17-11-3a14-3a53.png)

   Congratulations! You have successfully edited your workspace!

>[!NOTE]
>
>**Deep Dive**
>
>[Understanding Workspaces and Person Partitions](understanding-workspaces-and-person-partitions.md).

