...
The Weekly Activity Usage dashboard provides a weekly count of Activity Types over a rolling 52-week period. Weekly activities produced are a good indicator of how much marketing you are doing in Marketo Engage. Activities serve as a proxy for the various system processes and trackable events that take place within Marketo.
...
It is contingent upon what activities are included in the pipeline.
...
This section provides counts of activities over the previous seven days based on meaningful slices of the data. Group activities by the most common activity types seen in the last seven days. This may include categories like _Change Data Value_, _Add to List_, or _Send Email_. This lets you see what types of activities are happening most often in the system. Activity Type usage is a key indicator of determining growth, or if optimizations are needed to reduce usage.

>[!NOTE]
>
>* All the breakdowns below are a rolling seven-day sum and do **not** include the current day. Consider it as "yesterday + six days before that."
>
>* The dashboard only shows the top 20 activity types, while the rest are sorted into a category titled "Other."

Activity usage shows you how much marketing is being conducted, and helps visualize growth against identified for the level of product contracted for. Dashboards can also be used as a guide to determine how much optimization can or should be done by reducing the fields being updated.
...
_Change Data Value_ is the most common activity type. It indicates when a piece of information on a person/lead record is updated. Group by the fields that are changed most often to determine what information is useful for your marketing operations and whether there are opportunities to optimize platform usage.

### By Campaign {#by-campaign}

Group by which campaigns are producing the most activities. This allows you to see if you have any particularly "noisy" campaigns that create more activity than necessary. Learn about campaigns that should be decommissioned, or campaigns that are doing more work than intended.
