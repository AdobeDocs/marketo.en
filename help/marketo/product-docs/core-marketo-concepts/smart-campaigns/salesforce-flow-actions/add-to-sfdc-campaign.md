...
This flow step can be used in Marketo campaigns or as a single flow step to add people as leads in a [!DNL Salesforce] campaign. If the lead does not yet exist in [!DNL Salesforce], it will automatically be synced over and added to the campaign with the specified status.
...
1. Find and select the [!DNL Salesforce] campaign you want to add your leads to.
   ...
   >[!TIP]
   >
   >If you can't see a [!DNL Salesforce] campaign in the Campaign list:
   >
   ...
   >[!TIP]
   >
   >You can use [!DNL Salesforce] campaign [My Tokens](/help/marketo/product-docs/core-marketo-concepts/programs/tokens/managing-my-tokens.md) to make program cloning easier.
...
1. Select the [!DNL Salesforce] campaign member status you want to assign to leads when they're added.
   ...
   >[!CAUTION]
   >
   >If a person is already a lead member of the [!DNL Salesforce] campaign, they will be skipped and their status will NOT be updated. You can use [change their status in an SFDC campaign](/help/marketo/product-docs/core-marketo-concepts/smart-campaigns/salesforce-flow-actions/change-status-in-sfdc-campaign.md) instead.
