...
product: marketo
title: Conditional Email Open Tracking
description: Learn how to configure conditional email open tracking using a custom boolean field to route email open tracking based on each person's consent status.
level: Beginner, Intermediate
feature: Email Designer
---
# Conditional email open tracking {#conditional-open-tracking}

Learn how to configure Marketo Engage to honor end-user consent for email open (pixel) tracking, in alignment with [various guidelines](https://experienceleaguecommunities.adobe.com/adobe-marketo-engage-general-27/understanding-guidance-on-email-tracking-pixels-251632){target="_blank"}. The approach uses a custom boolean field to determine which email variant a person receives, one with open tracking enabled or one with it disabled.
