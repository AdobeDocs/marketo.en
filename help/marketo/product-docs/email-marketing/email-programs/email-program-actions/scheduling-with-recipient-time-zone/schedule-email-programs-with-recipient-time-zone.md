...
There are two potential scenarios when scheduling an email program while the Recipient Time Zone is enabled:

1. Scheduling the program to run **within** the next 25 hours
1. Scheduling the program to run **more** than 25 hours in the future (that is, next week)
...
9:00am has already passed in New York and Italy, so qualified people in these two time zones receive the email based on the **Time Zone Settings**:

* **[!UICONTROL Deliver the following day in the recipient's time zone]:** Wednesday at 9:00am in their respective time zones, **OR**
...
In short, email programs scheduled with Recipient Time Zone need to start running at the scheduled time in the earliest time zone (that is, where it reaches midnight first) to accommodate every time zone. So, if you schedule an email program...

* **with a delivery time *within* 25 hours**, the program starts running within 15 minutes. Recipients who have already passed the scheduled time receives the email based on the time zone settings you've chosen.
