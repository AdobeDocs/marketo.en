...
* **Get Program Member API Fix**: A change was made recently to correct the behavior of the [Get Program Members](https://developers.marketo.com/rest-api/endpoint-reference/lead-database-endpoint-reference/#!/Program_Members/getProgramMembersUsingGET){target="_blank"} endpoint. Previously, when utilizing the `updatedAt` filter type to specify a date range, there was a chance that program membership records updated within that range were not included in the response. Additionally, there was a chance that program membership records updated outside of the specified date range being incorrectly included in the response. Both issues have been resolved.
