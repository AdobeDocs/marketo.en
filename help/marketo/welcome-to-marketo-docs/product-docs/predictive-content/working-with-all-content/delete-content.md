---
unique-page-id: 11384659
description: Delete Content - Marketo Docs - Product Documentation
title: Delete Content
---

# Delete Content {#delete-content}

Delete Content - Marketo Docs - Product Documentation

>[!NOTE]
>
>Depending on the purchase date, your Marketo subscription may include either Marketo Predictive Content or Content`<sup>AI</sup>`. For those using Predictive Content, Marketo is enabling Content`<sup>AI</sup>` Analytics features until April 30, 2018. To keep these features beyond that date, please contact your Marketo Customer Success Manager to upgrade to Marketo Content`<sup>AI</sup>`.

When you don't need a piece of content anymore, it's easy to get rid of it. 

##### 1. Check the box next to the piece of content you want to remove. {#deletecontent-checktheboxnexttothepieceofcontentyouwanttoremove.}

![](assets/image2017-10-3-9-3a8-3a39.png)

##### 2. Click the Content Actions drop-down and select Delete Content. {#deletecontent-clickthecontentactionsdrop-downandselectdeletecontent.}

![](assets/image2017-10-3-9-3a9-3a12.png)

That's it.
